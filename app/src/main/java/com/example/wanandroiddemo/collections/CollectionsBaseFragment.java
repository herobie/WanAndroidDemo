...
public class CollectionsBaseFragment extends Fragment {
    ...
    public static CollectionsBaseFragment getInstance(String param){
        CollectionsBaseFragment collectionsBaseFragment = new CollectionsBaseFragment();
        Bundle args = new Bundle();
        args.putString(Constant.ARG_PARAM1, param);
        collectionsBaseFragment.setArguments(args);
    ...
    public void initView(){
        ...
        TabLayoutMediator mediator = new TabLayoutMediator(binding.collectionsTab, binding.collectionsViewPage, (tab, position) -> {
            if (position == 0){
                tab.setText("收藏文章");
            }else if (position == 1){
                tab.setText("收藏网站");
        ...
        articleViewModel.getIsSuccess().observe(getViewLifecycleOwner(), aBoolean -> {
